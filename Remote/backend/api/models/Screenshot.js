...
const screenshotSchema = new mongoose.Schema(
  {
    ...
    source: {
      type: String,
      enum: ["browser", "desktop", "manual", "fullscreen"],
      default: "browser",
    },
...
screenshotSchema.pre("remove", async function (next) {
  const fs = require("fs");
  ...
  if (this.filePath) {
    if (this.filePath.startsWith("http")) {
      // Cloudinary delete
      try {
        const publicId = this.filePath.split("/").pop().split(".")[0];
        await cloudinary.uploader.destroy(`remote-tracker/screenshots/${publicId}`);
      } catch (error) {
