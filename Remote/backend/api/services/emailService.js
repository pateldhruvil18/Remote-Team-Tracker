...
class EmailService {
  ...
  async sendOTP(user, otp) {
    const subject = "Email Verification - Team Tracker";
    const html = this.getOTPTemplate(user, otp);
  ...
  async sendEmail(to, subject, html) {
    try {
      ...
      const isRealEmail = this.isUsingRealEmail;
    ...
    return `
      ...
      <head>
        <style>
          ...
          .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
          .otp-code { background: #e8e8e8; padding: 15px; border-radius: 6px; margin: 15px 0; text-align: center; font-size: 28px; font-weight: bold; letter-spacing: 5px; color: #000; }
          .warning { background: #fff3cd; padding: 15px; border-radius: 6px; border-left: 4px solid #ffc107; margin: 15px 0; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="header">
            ...
            <p>Your one-time password for Team Tracker</p>
          </div>
          <div class="content">
            <p>Hi ${user.firstName},</p>
            <p>Thank you for registering with Team Tracker. Please use the following One-Time Password (OTP) to verify your email address:</p>
